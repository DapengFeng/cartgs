...
void LoopClosing::CorrectLoop() {
  ...
  std::cout << "Loop: mpCurrentKF -> " + to_string(mpCurrentKF->mnId)
            << std::endl;
  ...
  std::unordered_set<unsigned long>
      LoopKeyFrameIds;  // record loop closure keyframes
  ...
  if (pLoopMap->IsInertial() && pLoopMap->isImuInitialized()) {
    Optimizer::OptimizeEssentialGraph4DoF(
        ...
        LoopConnections, opr, LoopKeyFrameIds);
  } else {
    // cout << "Loop -> Scale correction: " << mg2oLoopScw.scale() << endl;
    Optimizer::OptimizeEssentialGraph(
        pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3,
        LoopConnections, bFixedScale, opr, LoopKeyFrameIds);
  }
#ifdef REGISTER_TIMES
