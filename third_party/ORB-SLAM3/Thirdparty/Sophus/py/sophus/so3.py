...
class So3:
    ...
    def exp(v):
        ...
        return So3(
            sophus.Quaternion(
                sympy.cos(0.5 * theta), sympy.sin(0.5 * theta) / theta * v
            )
        )
    ...
    def vee(Omega):
        v = sophus.Vector3(
            Omega.row(2).col(1), Omega.row(0).col(2), Omega.row(1).col(0)
        )
        return v
    ...
    def __mul__(self, right):
        """Left-multiplication either rotation concatenation or point-.
    ...
    def calc_Dx_this_mul_exp_x_at_0(self, x):
        return (
            sympy.Matrix(
                4, 3, lambda r, c: sympy.diff((self * So3.exp(x))[r], x[c])
            )
            .subs(x[0], 0)
            .subs(x[1], 0)
            .limit(x[2], 0)
    ...
    def calc_Dx_exp_x_mul_this_at_0(self, x):
        return (
            sympy.Matrix(
                3, 4, lambda r, c: sympy.diff((self * So3.exp(x))[c], x[r, 0])
            )
            .subs(x[0], 0)
            .subs(x[1], 0)
            .limit(x[2], 0)
    ...
    def calc_Dxi_x_matrix(x, i):
        return sympy.Matrix(
            3, 3, lambda r, c: sympy.diff(x.matrix()[r, c], x[i])
        )
...
class TestSo3(unittest.TestCase):
    ...
    def test_matrix(self):
        ...
        self.assertEqual(sympy.simplify(p1_foo - p2_foo), sophus.ZeroVector3())

    def test_derivatives(self):
        self.assertEqual(
            sympy.simplify(
                So3.calc_Dx_exp_x_at_0(self.omega) - So3.Dx_exp_x_at_0()
            ),
        ...
        for i in [0, 1, 2, 3]:
            self.assertEqual(
                sympy.simplify(
                    So3.calc_Dxi_x_matrix(self.a, i)
