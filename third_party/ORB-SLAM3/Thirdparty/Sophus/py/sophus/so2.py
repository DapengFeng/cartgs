...
class So2:
    ...
    def exp(theta):
        """Exponential map."""
        return So2(sophus.Complex(sympy.cos(theta), sympy.sin(theta)))
    ...
    def matrix(self):
        ...
        return sympy.Matrix(
            [[self.z.real, -self.z.imag], [self.z.imag, self.z.real]]
        )
    ...
    def calc_Dxi_x_matrix(x, i):
        return sympy.Matrix(
            2, 2, lambda r, c: sympy.diff(x.matrix()[r, c], x[i])
        )
...
class TestSo2(unittest.TestCase):
    ...
    def test_derivatives(self):
        self.assertEqual(
            sympy.simplify(
                So2.calc_Dx_exp_x_at_0(self.theta) - So2.Dx_exp_x_at_0()
            ),
        ...
        for i in [0, 1]:
            self.assertEqual(
                sympy.simplify(
                    So2.calc_Dxi_x_matrix(self.a, i)
