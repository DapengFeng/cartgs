...
__global__ void search_neighborhood_to_estimate_depth_and_reproject_pinhole(
    ...
    const bool* __restrict__ has3D,
    const float* __restrict__ point3D_orig,
    const float* __restrict__ colors,
    float* __restrict__ point3D_result,
    float* __restrict__ colors_result) {
  auto idx = cg::this_grid().thread_rank();
  if (idx >= N) return;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (dist > max_pixel_dist || dist >= min_dist) continue;
    // else
    min_dist = dist;
  ...
  if (depth > 0.0f) {
    float3 pt_result = reproject_depth_pinhole(u, v, depth, fx, fy, cx, cy);
...
monocularPinholeInactiveGeoDensifyBySearchingNeighborhoodKeypoints(
  ...
  if (N != 0) {
    ...
    torch::Tensor depth_valid_flags = torch::where(
        result_pt.index({torch::indexing::Slice(), 2}) > 0.0f, true, false);
